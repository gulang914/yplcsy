...
use App\Model\Test\Course\SamplingTime;
...
class SamplingTimeController extends Controller
{
    ...
    {
        $this->tableName = (new SamplingTime)->getTable();
    }
    /**
     * Index interface.
     *
    ...
    {
        return Admin::grid(SamplingTime::class, function (Grid $grid) {
    ...
    {
        $show = new Show(SamplingTime::findOrFail($id));

        $show->test('项目名称',function ($test){
            $test->shiyan_name('试验名称');
        });
    ...
    {
        return Admin::form(SamplingTime::class, function (Form $form) {
